...
public class CopyOfTestOBL extends GridSim{
...
*/
	public CopyOfTestOBL(String name, double baudRate, int totalResource) throws Exception {
		super(name, baudRate);
		this.name_=name;
	...
	private void schedule(int[] resourceID, ResourceCharacteristics[] resChars) {
		...
		for(i=0; i<GRIDLET_NUM; i++){
			...
			//TODO 发现逻辑没有那么简单，首先要将资源按处理能力排序，然后轮询资源，找到空闲的且处理能力强的资源
			if(resChars[2].getNumFreePE()!=0){
				index=2;
			}else if(resChars[1].getNumFreePE()!=0){
				index=1;
			}else{
				index=0;
			}
...
*/			GridResource resource0=createGridResource("Resource_0", 30);
			...
			CopyOfTestOBL user0=new CopyOfTestOBL("User_0", 560.00, total_resource);//创建网格用户，同时生成了任务集合
			CopyOfTestOBL user1=new CopyOfTestOBL("User_1", 560.00, total_resource);//创建网格用户，同时生成了任务集合
			CopyOfTestOBL user2=new CopyOfTestOBL("User_2", 560.00, total_resource);//创建网格用户，同时生成了任务集合
