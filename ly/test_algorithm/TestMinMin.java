...
/*
 * Min-Min算法
 * 计算出ECT矩阵，找出最早完成时间最小的任务资源对，将任务发送至该资源处理；
 * 更新ECT矩阵，算上资源的就绪时间；
 * 再进行一轮比较，选出新的最早完成时间最小的任务资源对；
 * 循环，直到任务集合为空
 * 
 * 实验条件：
...
*/	private double[] glLengths={569.0, 1019.0, 949.8, 481.7, 226.4, 827.1, 897.8,
	...
	private void schedule(int[] resourceID, ResourceCharacteristics[] resChars) {
		...
		while(submitted.size()<GRIDLET_NUM){
			/*
			 * 若已提交的任务数小于任务总数（即还有任务需要调度）：
			 * 1.遍历ect矩阵，找到完成时间最短的任务资源对（完成时间=ect[i, j]+ready[j]）
			 * （用到一个存储最小时间值的变量和两个分别存储i、j的变量）
			 * 2.更新资源j的就需时间，即ready[j]
			 * 3.将任务i发送至j资源执行
			 * 4.将任务i的下标存储到submitted集合里
			 * 5.进行下一次循环
			 */
			double minTime=Double.MAX_VALUE;//存储最小时间值
			int m=0;//临时存储i
			int n=0;//临时存储j
			for(i=0; i<ect.length; i++){
				if(submitted.contains(i)) continue;//希望的是如果任务序号在已经提交的任务集合里，则不需要进行下面代码，直接进入下次循环
				for(j=0; j<totalResource_; j++){
					double cTime=ect[i][j]+ready[j];
					if(cTime<minTime){
						minTime=cTime;
						m=i;
						n=j;
