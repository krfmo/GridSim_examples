...
/*
 * MET算法，即最小执行时间算法
 * 将任务以随机的顺序发送至执行时间最小的资源
 * 执行时间仅指资源处理任务所需的时间
 * 因此该算法本质是将任务发送给处理速度最快的资源，因此会导致负载不均衡
 * 
 * 实验条件：
 * 1.三个资源，每个资源只有一个Machine，每个Machine只有一个PE
 * 2.每个资源的处理速度各不相同，mipsRating分别为37，39，41，第三个资源处理速度最快（因此，在MET算法测试中，所有任务都将发送至第三个资源处理）
...
public class TestMET extends GridSim{
...
*/	private double[] glLengths={569.0, 1019.0, 949.8, 481.7, 226.4, 827.1, 897.8,
	...
	public TestMET(String name, double baudRate, int totalResource) throws Exception {
		super(name, baudRate);
		this.name_=name;
	...
	public static void main(String[] args) {
		...
		try {
			...
			TestMET user0=new TestMET("User_0", 560.00, total_resource);//创建网格用户，同时生成了任务集合
			TestMET user1=new TestMET("User_1", 560.00, total_resource);//创建网格用户，同时生成了任务集合
			TestMET user2=new TestMET("User_2", 560.00, total_resource);//创建网格用户，同时生成了任务集合
