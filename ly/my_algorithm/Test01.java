...
public class Test01 extends GridSim{
	...
	public static final float PER_CENTAGE=0.5f;
	...
	/**
	 * 算法选择部分，根据任务长度、长短任务比例、机器间处理能力差异等决定采用何种算法
	 * @param resourceID
	 * @param resChars
	 * @throws Exception 
	 */
	private void schedule(int[] resourceID, ResourceCharacteristics[] resChars) {
		...
		//step2:循环调度每个任务，并且根据具体情况制定调度该任务应使用的调度算法
		for(i=0; i<GRIDLET_NUM; i++){
			int sch=0;//调度资源的下标
	...
	/**
	 * 自己的算法
	 * 根据机器处理能力、任务长度、长短任务比例等条件，选择最佳的调度算法
	 * @return 返回值代表使用的算法的字符串
	 */
	private String myAlgorithm() {
		double meanValue=(MIPS_RATING_1+MIPS_RATING_2+MIPS_RATING_3)/3.0;//机器处理能力均值
		double sqr=Math.sqrt(((MIPS_RATING_1-meanValue)*(MIPS_RATING_1-meanValue)+
				(MIPS_RATING_2-meanValue)*(MIPS_RATING_2-meanValue)+
				(MIPS_RATING_3-meanValue)*(MIPS_RATING_3-meanValue))/3.0);//处理能力均方根，标准差
		if(sqr>10){//机器间处理能力差异较大
			if(){
				return "MET";//处理能力差异较大，且任务长度较小时使用MET算法
			}
		}
